...
class IncomeServiceImplTest {
    ...
    @Test
    public void testSaveIncome() throws  AccountingConstraintViolationException, NoSuchEntityOrNotAuthorized {
    ...
    @Test
    void testSave_WithZeroAmount_ShouldThrowAccountingConstraintViolationException() {

        IncomeDTO incomeDTO = new IncomeDTO();
        incomeDTO.setAmount(0);
        IncomeType incomeType = new IncomeType();

        assertThrows(AccountingConstraintViolationException.class, () -> incomeService.save(incomeDTO, incomeType));
        verifyNoInteractions(incomeTypeRepository);
    ...
    @Test
    void testSave_WithInvalidIncomeType_ShouldThrowNoSuchEntityOrNotAuthorized() {

        IncomeDTO incomeDTO = new IncomeDTO();
        incomeDTO.setAmount(100);
        IncomeType incomeType = new IncomeType();
        incomeType.setId(1L);
        when(incomeTypeRepository.findById(1L)).thenReturn(Optional.empty());

        assertThrows(NoSuchEntityOrNotAuthorized.class, () -> incomeService.save(incomeDTO, incomeType));
        verify(incomeTypeRepository, times(1)).findById(1L);
        verifyNoInteractions(securityUtil);
        verifyNoInteractions(incomeRepository);
        verifyNoInteractions(walletRepository);
    }
    ...
    @Test
    void testSave_WithExceptionOnSave_ShouldReturnFalse() throws AccountingConstraintViolationException, NoSuchEntityOrNotAuthorized {

        long incomeTypeId = 1L;
        IncomeDTO incomeDTO = new IncomeDTO();
        incomeDTO.setAmount(100);
        IncomeType incomeType = new IncomeType();
        incomeType.setId(1L);
        ...
        when(securityUtil.getLoggedInUser()).thenReturn(userEntity);
        when(incomeTypeRepository.findById(incomeTypeId)).thenReturn(Optional.of(incomeType));
        when(securityUtil.getLoggedInUser().getWallet()).thenReturn(userEntity.getWallet());
        doThrow(Exception.class).when(incomeRepository).save(any(Income.class));
        boolean result = incomeService.save(incomeDTO, incomeType);
